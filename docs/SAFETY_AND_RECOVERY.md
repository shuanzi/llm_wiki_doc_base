...
- `watch` 每次全量重扫输入目录；队列、lease 和 Agent 输出只进入 Binding Runtime。Agent 按 Source 独立操作临时 Vault 副本，只有 Source/Hash/Log/Doctor/结构化结果全部通过才以带 Vault identity 的可恢复事务发布；`retry`、`needs-review`、`permanent-error` 或崩溃不会把临时修改留在 Durable Vault。队列丢失后从仍为 `registered` 的 Source Record 恢复，不把 Agent 退出码当作 Ingest 成功；
