...
- 提供 14 个 MCP 工具：11 个 workflow 原子能力 + 3 个 maintenance 能力。
...
- 以 stdio MCP server 暴露 14 个工具及 JSON Schema。
...
src/
  mcp_server.ts
  tools/kb_*.ts            # 11 个 workflow tool + 3 个 maintenance tool
...
4. `kb_read_source`
- 作用：按 `source_id` 读 manifest，再读 canonical Markdown source content。
...
- 关键行为：要求 `kb_root` 位于某个 git working tree 内；提交前和 `git add -A -- <kb_root scope>` 后都会拒绝已 staged 的 `kb_root` scope 外文件，避免把无关暂存内容带入同次 commit。
...
- `scripts/e2e_v2_ingest.ts`
  - 覆盖当前 ingest/read/write/index/log/finalize/commit guard 的端到端链路；tool surface contract 由 `scripts/validate_kb_tool_contract_baseline.ts` 校验 14-tool canonical surface、`kb_url_add` 顺序和工具 schema。
...
5. `kb_run_lint` / `kb_repair` 已成为独立 MCP 工具，但 README 与流程文档仍需持续保持与 canonical tool surface 同步，避免再次出现过期工具数量描述。
