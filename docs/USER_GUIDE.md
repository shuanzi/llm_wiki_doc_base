...
需要自动处理 drop folder 时，可让系统调度器每 30 分钟运行一次 `llm-wiki watch`。该命令每次进行全量扫描，先完成确定性注册，再启动 ephemeral Codex Agent 执行语义 Ingest；完整前置条件、恢复语义和跨平台调度示例见 [周期扫描与自动 Ingest](WATCHER.md)。
