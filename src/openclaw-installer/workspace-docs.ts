...
function buildToolsDocContent(): string {
  ...
  lines.push(
    ...
    "frontmatter `id` 是 wikilink 的 canonical target；文件名可以不同，避免把文件名 stem 当作 page_id。",
    "`kb_ensure_entry.entry` 不要包含 `<!-- dedup:... -->`；工具会根据 `dedup_key` 自动追加 dedup 注释。`anchor: null` 或 `anchor: \"\"` 都表示追加到文件末尾。",
    "`kb_append_log_entry.references` 只传可解析的 wiki page_id/wikilink 或已有 manifest 的 source_id；source_id 会解析到对应 source summary page；wikilink label 不得包含 `[[`、`]]` 或 `|`；无法解析时工具会拒绝写入，避免生成 broken wikilink。",
    "",
    "## 实战工作流",
