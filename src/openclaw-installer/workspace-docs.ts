...
const INSTALLER_COMMAND_SUMMARIES: Record<InstallerCommandName, string> = {
  install:
    "落地 installer-owned 工件，确保 `llmwiki` 会话可见 canonical `kb_*` 工具，并写入 manifest/MCP 状态。",
  check:
    "校验 `llmwiki` 会话可见 `kb_*`、ownership、运行时一致性与漂移。",
  repair:
...
function buildAgentsDocContent(): string {
  return renderMarkdown([
    ...
    "2. Installer 命令仅作用于显式 `--workspace`，且该 workspace 必须绑定 `llmwiki`，否则 fail-closed。",
    "3. `check` 以 `llmwiki` session-visible canonical `kb_*` 可用性为主判据；仅保存 MCP 配置不足以代表可用。",
...
function buildHeartbeatDocContent(): string {
  return renderMarkdown([
    ...
    "- [ ] 确认命令包含显式 `--workspace`，并绑定到 `llmwiki`。",
    "- [ ] 确认 `KB_ROOT` 指向已安装 `kb` 目录本体（`<KB_ROOT>/raw|wiki|schema|state`）。",
    "- [ ] 明确本次成功标准是 `llmwiki` session-visible canonical `kb_*`，不是“配置文件里有 MCP”。",
    "",
    "## 执行",
...
function buildToolsDocContent(): string {
  ...
  lines.push(
    ...
    "5. installer 生命周期：`install/check/repair/uninstall` 的主判据是 `llmwiki` 会话可见 canonical `kb_*`。",
...
function buildSoulDocContent(): string {
  return renderMarkdown([
    ...
    "4. 外部 `KB_ROOT` 是唯一知识写入边界，成功安装以 `llmwiki` session-visible canonical `kb_*` 为准。",
    "",
    "## 一致性与保守所有权",
    ...
    "",
    "## 修复原则",
    "1. `repair` 只重建 installer-owned 状态（含 llmwiki session-visible 运行时工件），不做猜测性迁移。",
