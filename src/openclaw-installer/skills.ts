...
function buildIngestSkillContent(): string {
  return renderSkillDocument(
    ...
    [
      ...
      "3. `kb_read_source(source_id)` 读取 canonical Markdown source content",
      "4. 理解 source content 的核心内容、关键实体和概念",
      "",
      "### 第 2 步：分析与规划（先向用户报告）",
      "",
      "- 文档概述（2-3 句话）",
      ...
      "2. 使用 `kb_append_log_entry` 在 `wiki/log.md` 追加本次 ingest 日志",
      "3. 使用 `kb_ingest_finalize` 将 source manifest 标记为 `ingested`；若整合失败则标记为 `failed` 并写明原因",
      "",
      "### 约束",
      "",
      "- 当前 OpenClaw 成功契约是 installer-configured OpenClaw agent（默认 `--agent-id` 为 `llmwiki`）会话内可直接使用 canonical `kb_*` 工具；若工具缺失，先运行 installer `check`/`repair`，不要把仅有 MCP 配置视为可用。",
      ...
      "- 若 DNS 返回 198.18.0.0/15、private 或 special-use IP，说明当前 DNS resolver/proxy 链路把公开域名解析到了非公网地址；安全检查会 fail-closed，应修运行环境而不是绕过校验。",
      "- `kb_write_page` 内容必须包含完整 YAML frontmatter：`id`、`type`、`title`、`updated_at`、`status`。",
      "- wikilink 目标必须优先使用目标页 frontmatter `id`；文件名可以不同，不要把文件名 stem 当作 page_id。",
...
function buildQuerySkillContent(): string {
  return renderSkillDocument(
    ...
    [
      ...
      "### 回答问题",
      "",
      "1. `kb_search_wiki(query)` 搜索相关页面（可组合关键词与过滤条件）；召回不足时使用 `mode: \"chunk\"` 做全文 chunk 搜索",
      ...
      "- 产出综合性结论并给出引用",
      "",
      "记录方式：`kb_append_log_entry` 追加 query 日志，并使用唯一 `run_id` 防重。",
      "",
      "### 结果回写（可选）",
      "",
      "当回答具备长期价值时，可在用户同意后：",
      ...
      "3. `kb_append_log_entry` 记录同一 run_id 的 query 日志",
      "",
      "### 约束",
      "",
      "- 当前 OpenClaw 成功契约是 installer-configured OpenClaw agent（默认 `--agent-id` 为 `llmwiki`）会话内可直接使用 canonical `kb_*` 工具；若工具缺失，先运行 installer `check`/`repair`，不要把仅有 MCP 配置视为可用。",
      ...
      "- wikilink 目标必须优先使用目标页 frontmatter `id`；文件名可以不同，不要把文件名 stem 当作 page_id。",
      "- `kb_append_log_entry.references` 只传可解析的 wiki page_id/wikilink 或已有 manifest 的 source_id；wikilink label 不得包含 `[[`、`]]` 或 `|`；避免写入 broken wikilink。",
...
function buildLintSkillContent(): string {
  return renderSkillDocument(
    ...
    [
      ...
      "### 第 4 步：记录 lint 日志",
      "",
      "完成完整 lint pass 后，用 `kb_append_log_entry` 追加 `wiki/log.md`：",
